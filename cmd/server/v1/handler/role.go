...
func (c RoleCtrl) Paginate(ctx iris.Context) {
	req := &v1.RolePageReq{}
	...
	result, err := c.RoleService.Paginate(*req)
	if err != nil {
...
func (c RoleCtrl) Get(ctx iris.Context) {
	...
	role, err := c.RoleService.Get(id)
	if err != nil {
		ctx.JSON(_domain.Response{Code: _domain.FailErr.Code, Msg: err.Error()})
		return
	}

	ctx.JSON(_domain.Response{Code: _domain.Success.Code, Data: role})
}
