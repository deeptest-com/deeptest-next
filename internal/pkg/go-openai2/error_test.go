...
func TestAPIErrorUnmarshalJSON(t *testing.T) {
	type testCase struct {
		...
		response  string
		hasError  bool
		checkFunc func(t *testing.T, apiErr openai2.APIError)
	}
	testCases := []testCase{
		...
		},
		{
			...
			hasError: false,
			checkFunc: func(t *testing.T, apiErr openai2.APIError) {
				assertAPIErrorInnerError(t, apiErr, &openai2.InnerError{
					Code: "ResponsibleAIPolicyViolation",
					ContentFilterResults: openai2.ContentFilterResults{
						Hate: openai2.Hate{
							Filtered: false,
							Severity: "safe",
						},
						SelfHarm: openai2.SelfHarm{
							Filtered: false,
							Severity: "safe",
						},
						Sexual: openai2.Sexual{
							Filtered: true,
							Severity: "medium",
						},
						Violence: openai2.Violence{
							Filtered: false,
							Severity: "safe",
						},
					},
...
func assertAPIErrorMessage(t *testing.T, apiErr openai2.APIError, expected string) {
	if apiErr.Message != expected {
		t.Errorf("Unexpected APIError message: %v; expected: %s", apiErr, expected)
	}
}

func assertAPIErrorInnerError(t *testing.T, apiErr openai2.APIError, expected interface{}) {
	if !reflect.DeepEqual(apiErr.InnerError, expected) {
		t.Errorf("Unexpected APIError InnerError: %v; expected: %v; ", apiErr, expected)
	}
}

func assertAPIErrorCode(t *testing.T, apiErr openai2.APIError, expected interface{}) {
...
func assertAPIErrorParam(t *testing.T, apiErr openai2.APIError, expected *string) {
	if apiErr.Param != expected {
		t.Errorf("Unexpected APIError param: %v; expected: %s", apiErr, *expected)
	}
}

func assertAPIErrorType(t *testing.T, apiErr openai2.APIError, typ string) {
	if apiErr.Type != typ {
...
func TestRequestError(t *testing.T) {
	var err error = &openai2.RequestError{
