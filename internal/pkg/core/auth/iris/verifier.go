...
func Get(ctx *context.Context) *auth.MultiClaims {
	if v := ctx.Values().Get(claimsContextKey).(*auth.MultiClaims); v != nil {
		return v
	}
	return nil
}
...
func GetAuthorityId(ctx *context.Context) []string {
	if v := Get(ctx); v != nil {
		return strings.Split(v.AuthorityId, auth.AuthorityTypeSplit)
	}
	return nil
}
...
func IsRole(ctx *context.Context, authorityType int) bool {
	...
	b, err := auth.AuthDriver.IsRole(string(v), authorityType)
	if err != nil {
		return false
	}
	return b
...
func IsAdmin(ctx *context.Context) bool {
	return IsRole(ctx, auth.AdminAuthority)
}
...
func NewVerifier(validators ...auth.TokenValidator) *Verifier {
	return &Verifier{
...
func (v *Verifier) VerifyToken(token []byte, validators ...auth.TokenValidator) ([]byte, *auth.MultiClaims, error) {
	if len(token) == 0 {
		return nil, nil, auth.ErrEmptyToken
	}
	var err error
	...
	rcc, err := auth.AuthDriver.GetMultiClaims(string(token))
	if err != nil {
		return nil, nil, err
	}
...
func (v *Verifier) Verify(validators ...auth.TokenValidator) context.Handler {
