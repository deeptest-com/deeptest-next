...
func (c *customLogger) Printf(message string, data ...interface{}) {
	if CONFIG_MYSQL.LogZap != "" {
		switch len(data) {
		case 0:
			_logUtils.Info(message)
		case 1:
			_logUtils.Zap.Info("gorm", zap.Any("src", data[0]))
		case 2:
			_logUtils.Zap.Info("gorm", zap.Any("src", data[0]), zap.Any("duration", data[1]))
		case 3:
			_logUtils.Zap.Info("gorm", zap.Any("src", data[0]), zap.Any("duration", data[1]), zap.Any("rows", data[2]))
		case 4:
			_logUtils.Zap.Info("gorm", zap.Any("src", data[0]), zap.Any("duration", data[1]), zap.Any("rows", data[2]), zap.Any("sql", data[3]))
		}
		return
	}
	switch len(data) {
